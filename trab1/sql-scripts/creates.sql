...
(
	...
	primary key(ticket,requestDate)
)
...
go
create proc dbo.sp_Insert_Ticket_Action @ticket varchar(20), @technician int, @stepTicketType varchar(20), @stepOrderNumber int, @note varchar(100)
as
	...
	insert into dbo.Ticket_Action(beginDate,note,ticket,stepTicketType,stepOrderNumber,technician) values (getdate(),@note,@ticket,@stepTicketType,@stepOrderNumber,@technician);
go

-- terminar uma acção de um ticket
go
create proc dbo.sp_End_Ticket_Action @beginDate datetime, @ticket varchar(20)
as
	update dbo.Ticket_Action set endDate = getdate() where beginDate = @beginDate and ticket = @ticket;
go
...
as
	...
	insert into dbo.Request(requestDate,ticket,ticketUser) values(getdate(),@ticket,@ticketUser);

go

create proc dbo.sp_Answer_Ticket_Request @requestDate datetime, @ticket varchar(20), @technician int, @response varchar(max)
as
	...
	where requestDate = @requestDate and ticket = @ticket;
go
